...
export const DatabaseBrowser = ({ modalHandler, tmpl }) => {
  const { setActiveTab, setActiveListItem, filterList, setSearchString } =
    useStoreActions();
    ...
    <div className={tmpl.db}>
      ...
      <div className={tmpl.dbBrowser}>
        <div className={tmpl.dbSearchCont}>
          <button className={tmpl.dbFilterBtn} onClick={modalHandler}>
            Filter
          </button>
          <input
            id='search'
          ...
          <div className={tmpl.dbPrevCol}>
            ...
            <ul className={tmpl.dbPrvSpecs}>
              <li className={tmpl.dbPrvSpecsItem}>
                <span className={tmpl.dbPrvSpecsItemStrong}>
                  C. {list[activeListItem].species}
                </span>
              </li>
              <li className={tmpl.dbPrvSpecsItem}>
                Plant:{" "}
                <span className={tmpl.dbPrvSpecsItemStrong}>
                  {list[activeListItem].plantSize}
                </span>
              </li>
              <li className={tmpl.dbPrvSpecsItem}>
                Fruit:{" "}
                <span className={tmpl.dbPrvSpecsItemStrong}>
                  {list[activeListItem].fruitSize}
                </span>
              </li>
              <li className={tmpl.dbPrvSpecsItem}>
                Spiciness:{" "}
                <span className={tmpl.dbPrvSpecsItemStrong}>
                  {list[activeListItem].heatLevel}
                </span>
              </li>
              <li className={tmpl.dbPrvSpecsItem}>
                Shape:{" "}
                <span className={tmpl.dbPrvSpecsItemStrong}>
                  {list[activeListItem].fruitShape}
                </span>
              </li>
              <li className={tmpl.dbPrvSpecsItem}>
                Color:{" "}
                <span className={tmpl.dbPrvSpecsItemStrong}>
                  {list[activeListItem].fruitColor}
                </span>
              </li>
            </ul>
