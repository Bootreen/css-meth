...
export const renderNewState = (oldState, sourceKey) =>
  oldState.map((e) => {
    const keyPair = {};
    const key = Object.keys(e)[0];
...
export const initialTabs = (() => {
  const newTabs = ["all"];
  chilies.forEach((e) => {
    if (!newTabs.includes(e.species)) newTabs.push(e.species);
  });
  return newTabs;
})();
...
export const fetchedShapeTraits = (() => {
  ...
  chilies.forEach((e) => {
    ...
    traits.shift();
    traits.forEach((e) => {
      if (!shapeTraits.includes(e)) shapeTraits.push(e);
    });
  });
...
export const fetchedColorTraits = (() => {
  ...
  chilies.forEach((e) => {
    ...
    traits.shift();
    traits.forEach((e) => {
      if (!colorTraits.includes(e)) colorTraits.push(e);
    });
  });
